...
return baseclass.extend({
	...
	makeTempTableContent() {
		...
		this.tempTable.append(
				E('tr', { 'class': 'tr' }, [
					E('td', { 'class': 'td left', 'data-title': _('Sensor') }, _('Fan')),
					E('td', { 'class': 'td left', 'data-title': _('Temperature') },
						(this.fanRpm === undefined || this.fanRpm === null) ? '-' : this.fanRpm + ' RPM'),
				])
			);
		...
		if(this.sensorsData && this.tempData) {
			for(let [k, v] of Object.entries(this.sensorsData)) {
				...
				const sensorNames = {
					'rfa_0_thermal': 'Czujnik RF 0',
					'rfa_1_thermal': 'Czujnik RF 1',
					'misc_thermal': 'Pozostałe',
					'cpu_top_thermal': 'CPU',
					'top_glue_thermal': 'SoC / Glue',
					'90000.mdio_1:01': 'PHY Ethernet',
				};
				sensor = sensorNames[sensor] || sensor;
					...
					for(let j of i.sources) {
						...
						let name = (j.label !== undefined) ? sensor + " / " + j.label :
							(j.item !== undefined && j.item !== "temp1_input") ? sensor + " / " + j.item.replace(/_input$/, "") : sensor
	...
	makeTempAreaContent() {
		...
		this.tempArea.append(
			E('div', { 'class': 'temp-status-list-item' }, [
				E('span', { 'class': 'temp-status-temp-value' },
					(this.fanRpm === undefined || this.fanRpm === null) ? '-' : this.fanRpm + ' RPM'
				),
				E('span', { 'class': 'temp-status-sensor-name' }, _('Fan'))
			])
		);
	...
	load() {
		...
		if(this.sensorsData) {
			return Promise.all([
				(this.sensorsPath.length > 0) ?
					L.resolveDefault(this.callTempData(this.sensorsPath), null) :
					Promise.resolve(null),
				L.resolveDefault(this.callFanStatus(), null),
			]);
		} else {
			return Promise.all([
	...
	render(data) {
		if(Array.isArray(data)) {
			if(data[0]) {
				if(data[0].sensors && !this.sensorsData) {
					this.sensorsData = data[0].sensors;
					this.sensorsPath = data[0].temp && new Array(...Object.keys(data[0].temp));
				}
				this.tempData = data[0].temp;
			}
			if(data[0])
				this.fanRpm = data[0].rpm;
		} else if(data) {
			if(!this.sensorsData) {
