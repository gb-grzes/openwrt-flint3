...
function renderStatus(node, running, status, history) {
	...
	dom.content(node, E('div', { 'class': 'fancontrol-status' }, [
		...
		E('div', { 'class': 'fancontrol-details' }, [
			...
			statusDetail(_('Fan device'), status.rpm_file || '-'),
...
return view.extend({
	...
	render() {
		...
		startTemp.description = _('Below this temperature the fan runs continuously at the configured minimum PWM. Unit: °C.');
		...
		startSpeed.default = '36';
		startSpeed.datatype = 'range(1,255)';
		startSpeed.rmempty = false;
		startSpeed.description = _('Minimum continuous PWM. The fan never stops; this value is used below the start temperature. Range: 1–255.');
