...
static int calculate_pwm(const struct fan_config *config, int64_t temp_mc)
{
	...
	/*
	 * Flint 3: fan must never stop.
	 * start_pwm is the minimum continuous PWM.
	 */
	if (temp_mc <= start_mc)
...
int main(int argc, char **argv)
{
	...
	while (!stop_requested) {
		...
		if (read_long_file(runtime.thermal_file, &raw_temp)) {
			...
			target_pwm = calculate_pwm(&config, temp_mc);
